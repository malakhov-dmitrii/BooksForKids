...
const WishlistItem = ({ item }: { item: IAmFavoriteItem }) => {
  ...
  return (
    <>
        ...
        <div className={`${styles.wishlist_item_cell} ${styles.wishlist_item_cell_product_name}`}>
            <div className={styles.wishlist_item_img}>
                <Image src={item.image} alt={item.image} width={100} height={100} className={styles.wishlist_item_image} />
            </div>
            <div className={styles.wishlist_item_title_container}>
                <Link href={`/catalog/${item.category}/${item.productId}`} className={styles.wishlist_item_title}>
                    <h5>
        ...
        <div className={styles.wishlist_item_cell}>
            {item.isDiscount
              ? <h5>
                    <span className={`line_through ${styles.wishlist_item_discount} ${styles.wishlist_item_price}`}>{formatPrice(+item.price)}</span><br />
                    <span className={styles.wishlist_item_price}>{`${formatPrice(+item.price * (1 - (+item.isDiscount)/100))}`}</span> 
              </h5>
        ...
        <div className={styles.wishlist_item_cell}>
            ...
            <div>
                ...
                <Link href='/cart' className={`black_btn ${styles.wishlist_btn} ${styles.wishlist_btn_view_cart}`}>
