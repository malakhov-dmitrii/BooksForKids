import RegistrationPage from '@/components/templates/registrationPage/RegistrationPage'
import clientPromise from '@/lib/mongodb'

import { generateTokens } from '@/lib/utils/api-routes'
import { OAuth2Client } from 'google-auth-library'

const redirectUri = 'http://localhost:3000/registration'
...
export default async function Registration(params: {
  ...
  if (!code) {
    return <RegistrationPage authUrl={authUrl} />
  }

  try {
    const dbTokens = await handleRegistration(code)
    return <RegistrationPage authUrl={authUrl} payload={dbTokens} />
  } catch (error) {
